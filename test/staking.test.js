...
contract("StakingRewardsFactory", ([owner, alice, bob, random]) => {
  ...
  before(async function () {
    ...
    stakingToken = await RewardsToken.new();
  ...
  it("should deploy staking rewards contract", async function () {
    await expectRevert(
      factory.deploy(stakingToken.address, REWARD_AMOUNT, REWARD_DURATION, { from: random }),
      "Ownable: caller is not the owner"
    );
    await factory.deploy(stakingToken.address, REWARD_AMOUNT, REWARD_DURATION, { from: owner });
    const stakingRewards = await factory.stakingRewardsInfoByStakingToken(stakingToken.address);
  ...
  it("should transfer reward tokens to staking contract", async function () {
    ...
    factory.notifyRewardAmounts({ from: random });
    const stakingRewards = await factory.stakingRewardsInfoByStakingToken(stakingToken.address);
    let stakingContractBalance = await etha.balanceOf(stakingRewards.stakingRewards);
    ...
    assert.equal(stakingContractBalance, fromWei(stakingRewards.rewardAmount));
  });

  it("should be able to stake staking token", async function () {
    let stakingRewards = await factory.stakingRewardsInfoByStakingToken(stakingToken.address);
